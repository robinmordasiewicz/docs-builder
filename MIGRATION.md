...
your-repo/
├── docs/
│   ├── index.mdx              # Landing page (REQUIRED, sidebar hidden)
│   ├── 01-overview.mdx        # Guide pages (autogenerated sidebar)
│   ├── 02-setup.mdx
│   └── 03-usage.mdx
├── .github/workflows/
...
- **`docs/index.mdx` is required.** It must contain a `title` in
  frontmatter and `sidebar: { hidden: true }` to keep it out of the
  sidebar. The builder extracts the title as the site title.
- **Guide pages go directly in `docs/`.** The sidebar auto-generates
  from all pages in `docs/`, sorted by filename prefix (`01-`, `02-`,
  etc.) or by the `sidebar.order` frontmatter field.
...
`docs/01-overview.mdx` ends up at
`builder/src/content/docs/01-overview.mdx`. To reach the builder's
`src/components/` directory, you go up 2 levels:

```
src/content/docs/01-overview.mdx
    ↑2     ↑1
```
...
You can organize content into subdirectories under `docs/`:
